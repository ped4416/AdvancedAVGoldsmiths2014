...
void testApp::audioRequested 	(float * output, int bufferSize, int nChannels){
	...
	for (int i = 0; i < bufferSize; i++){
        ...
        //AM, or amplitude modulation synthesis
        //float am = mySine.sinewave(440)*myOtherSine.sinewave(myPhasor.phasor(0.1,0,440));
        
        //or AM with musltiple sine tones.
        double wave=0;
        double frequency = 440;
        for(int i=0; i < 100; i++) {
            double thisSine = wave + sineBank[i].sinewave(frequency+ (i * frequency));
        ...
        //need a modulator(s) and a carrier wave. We only here the carrier wave with modulators effecting it.
        float carrier = int(mouseX)*2;//int(mouseY);//+envelope;//use this with an bool?
        float modulator = 0;//initialise a modulator to 0
        float modulator2 = 0;//initialise a modulator to 0
...
/*-----------------------------------------------------------------------*/
        ...
        //and finally output the result if single modulator is set to true!
        if (singleMod == true) {
...
/*-----------------------------------------------------------------------*/
        ...
        if (twoModualtorsSame == true) {
            //modulator2 set the same as first modulator..
            //output is simply the same but with a boost in volume and effect to carrier
            modulator = carrier;
        ...
        //other trials..
        //float fm = mySine.sinewave(myOtherSine.sinewave(myLastSine.sinewave(0.1)*30)*440);//awesome bassline
        //float fm = mySine.sinewave(440);
        //float fm = modulator*carrier;
        ...
        double setNoteSpeed = 0.5 + mouseY/100;
//        setNoteSpeed = 0.7;
//        setNoteSpeed = 0.9;
//        setNoteSpeed = 5;
        ...
        fmOut=VCF.lores(fm, 250+(ADSRout*100), 10);//now we stick the VCO's into the VCF, using the ADSR as the filter cutoff
